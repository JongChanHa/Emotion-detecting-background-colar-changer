...
sys.path.append(str(Path(__file__).resolve().parents[1] / 'common/python/openvino/model_zoo'))
...
def build_argparser():
    ...
    args.add_argument('-m', '--model', default="D:\Intel\openvino_test\open_model_zoo\demos\project\intel\modnet-webcam-portrait-matting\FP16\modnet-webcam-portrait-matting.xml",
...
def emotion_model(input_image):
    ...
    emotion_model_name = "D:\Intel\openvino_test\open_model_zoo\demos\project\intel\emotions-recognition-retail-0003\FP16-INT8\emotions-recognition-retail-0003.xml"
...
def face_detection_model(frame):
    ...
    face_detect_model_name = "D:\Intel\openvino_test\open_model_zoo\demos\project\intel\\face-detection-0200\FP16\\face-detection-0200.xml"
...
def render_results(frame, objects, output_resolution, target_bgr, person_id, blur_kernel=0, show_with_original_frame=False, emo=0):
    blur_kernel = tuple([blur_kernel] * 2) if blur_kernel else blur_kernel
    if target_bgr is None:
        ######################################
        target_bgr = cv2.blur(frame, blur_kernel) if blur_kernel else np.full(frame.shape, color_map(emo), dtype=np.uint8)
    else:
...
def main():
    ...
    while True:
        if pipeline.is_ready():
            ...
            # cv2.putText(results, 'emotion: {}'.format(face_d_test.emotion(emo)),
            #             (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0))
            #print(face_d_test.emotion(emo))
        ...
        if results:
            ...
            x1,x2,y1,y2 = face_d_test.face_detection_model(frame)
            face = frame[y1:y2,x1:x2]
            emo = face_d_test.emotion_model(face)


            frame = render_results(frame, objects, output_resolution, bgr, person_id,
                                   args.blur_bgr, args.show_with_original_frame, emo)
            ...
            if not args.no_show:
                cv2.putText(frame, 'emotion: {}'.format(face_d_test.emotion(emo)),
                         (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0))
