...
sys.path.append(str(Path(__file__).resolve().parents[1] / 'open_model_zoo\demos\common\python'))
sys.path.append(str(Path(__file__).resolve().parents[1] / 'open_model_zoo\demos\common\python\openvino\model_zoo'))
...
def build_argparser():
    ...
    args.add_argument('-m', '--model', default="public\modnet-webcam-portrait-matting\FP16\modnet-webcam-portrait-matting.xml",
...
def render_results(frame, objects, output_resolution, target_bgr, person_id, blur_kernel=0, show_with_original_frame=False, emo=0):
    ...
    if target_bgr is None:
        ######################################
        target_bgr = cv2.blur(frame, blur_kernel) if blur_kernel else np.full(frame.shape, face_emotion_color.emotion_color_map(emo), dtype=np.uint8)
    else:
...
def main():
    ...
    while True:
        ...
        if results:
            ...
            x1,x2,y1,y2 = face_emotion_color.face_detection_model(frame)
            face = frame[y1:y2,x1:x2]
            emo = face_emotion_color.emotion_model(face)
            ...
            if not args.no_show:
                cv2.putText(frame, 'emotion: {}'.format(face_emotion_color.emotion(emo)),
                         (10, 100), fontFace=4,fontScale=1, color=(0, 0, 0))
